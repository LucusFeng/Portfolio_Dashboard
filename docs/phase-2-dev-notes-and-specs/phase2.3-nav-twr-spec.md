...
- The daily node has **no `total` field**. Daily NAV must be **computed**:
  `daily_nav = cash + stock + dividendAccruals`. Verified: Sept-4 computed NAV = 33044.5024 =
  `ChangeInNAV.endingValue` to the cent.
...
- `parse_flex_daily_nav(xml)` → list of per-day rows: `account_external_id`, `report_date`
  (normalize `20251231`→`2026-...` wait: `20251231`→`2025-12-31`), `cash`, `stock`,
  `dividend_accruals`, and **computed `nav = cash + stock + dividend_accruals`**.
...
CREATE TABLE IF NOT EXISTS daily_nav (
    ...
    nav REAL NOT NULL,                 -- computed cash+stock+dividendAccruals (base ccy CAD)
    cash REAL, stock REAL, dividend_accruals REAL,
...
## Tests
- **Parse:** `parse_flex_change_in_nav` reads `twr=10.0905`, currency CAD, YTD dates;
  `parse_flex_daily_nav` returns 178 rows, computes `nav = cash+stock+dividendAccruals`, last
  row = 33044.50 = ChangeInNAV.endingValue.
